...
export async function depsCheck() {
  const dir = join(ROOT, "mcp-servers", "lex", "node_modules", "@modelcontextprotocol", "sdk");
  const present = await exists(dir);
  return {
    name: "MCP server dependencies",
    ok: present,
    version: present ? "installed" : "not installed",
    fix: "Run `make install` (or `npm install` in mcp-servers/lex).",
...
export async function indexStatus() {
  const dataDir = process.env.LEX_MCP_DATA_DIR || join(ROOT, ".data");
  const names = [
    ["ÚOOÚ guidance", "uoou.db", 30, "make index-uoou"],
    ["MŽP guidance and Věstník", "mzp.db", 90, "make index-mzp"],
    ["District and regional courts", "justice.db", 90, "make index-justice"],
  ];
  const out = [];
