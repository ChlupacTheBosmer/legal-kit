...
import { ROOT, exists, indexStatus } from "./lib/env.mjs";
import { bold, dim, green, yellow, cyan, ok, warn, rule } from "./lib/ui.mjs";
...
if (await exists(profile)) {
  ...
  ok(`PROFILE.md  ${dim(`${field("Name") || "(no name)"}, ${field("Role") || "role not stated"}`)}`);
  const areas = field("Main areas");
  if (areas) console.log(`        ${dim(`practises: ${areas}`)}`);
} else {
  warn(`PROFILE.md missing — run ${cyan("make setup")}`);
}
...
for (const [key, label] of [["mzp", "MŽP"], ["justice", "lower courts"], ["uoou", "ÚOOÚ"]]) {
  const log = join(ROOT, ".data", `${key}-build.log`);
...
console.log(`${dim("  Full check including network:")} ${cyan("make doctor")}\n`);
