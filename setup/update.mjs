...
import { ROOT, exists, indexStatus, pdftotextCheck } from "./lib/env.mjs";
import { bold, dim, green, yellow, cyan, ok, warn, bad, rule, todayISO, humanMs } from "./lib/ui.mjs";

const force = process.argv.includes("--force");
const checkOnly = process.argv.includes("--check");

const JOBS = {
  "uoou.db": { label: "ÚOOÚ guidance", args: ["tools/uoou-index.mjs", "build"], interval: 30 },
  "mzp.db": { label: "MŽP guidance and Věstník", args: ["tools/mzp-index.mjs", "build"], interval: 90 },
  "justice.db": {
...
if (checkOnly) {
  console.log(`\n${yellow(`${todo.length} index(es) need attention.`)} Run ${cyan("make update")}.\n`);
  process.exit(1);
}
...
rule("");
if (failures) {
  console.log(`${yellow(`${failures} build(s) did not finish.`)} They are resumable: run ${cyan("make update")} again.\n`);
  process.exit(1);
}
