...
const hasProfile = await exists(join(ROOT, "PROFILE.md"));
report({
  name: "PROFILE.md",
  ok: hasProfile,
  version: hasProfile ? "present" : "missing",
  fix: "Run `make setup` to write it, or copy PROFILE.example.md to PROFILE.md and edit.",
...
if (notBuilt) console.log(`${notBuilt} index(es) not built yet. ${cyan("make update")} builds them.`);
else if (stale) console.log(`${stale} index(es) are stale. ${cyan("make update")} refreshes them.`);
console.log(`\nRun the full end-to-end test against the live registries with ${cyan("make test")}.\n`);
