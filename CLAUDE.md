...
A working repository for answering legal questions from the official sources, and
for building a durable knowledge base from the answers. It reads Czech and
European Union primary law directly from the government registries rather than
from anyone's summary of them.

**Read `PROFILE.md` before anything else.** It says who you are working for, what
they practise, what their standing is, and which integrations are configured.
`PROFILE.md` is personal and is not tracked by git; this file holds only the rules
that apply to everybody. If `PROFILE.md` does not exist, say so and run
`make setup`.

This repository is not legal advice. Output is research: it states what the
sources say, how confident that reading is, and where a qualified advocate should
be brought in, naming the trigger and the specialism required.

Do not assume the reader may claim legal professional privilege over anything
here. Under § 21 of Act No. 85/1996 Coll., on the legal profession, the duty of
confidentiality attaches to the advocate (advokát), and Czech law has no attorney
work product doctrine because it has no United States style discovery. Whether the
reader is on the roll of advocates of the Czech Bar Association (Česká advokátní
komora, ČAK) is recorded in `PROFILE.md`.
...
5. **Czech law within the EU framework. Never US law.**

Details in "Writing style" and "Answer format" below. If the `claude-for-legal` plugins are
...
1. **Read `PROFILE.md`.** It is the user's own configuration: practice areas,
   standing, language, integrations. Without it you are guessing at the audience.
2. **Read `vault/50-Handbook/Agent guide.md`** — the practical companion to these
   rules: which tool to reach for, and the traps.
3. **Check `vault/50-Handbook/Maintenance log.md`.** If a task is past its due
   date, perform it before substantive work and update the log.
4. **Invoke the `task-observer` skill at the start of any task-oriented session**
   — before using tools to produce deliverables. It watches for patterns and
   corrections worth turning into skills.
5. For a legal question about Czech sources, invoke **`cz-legal-research`**.
...
The index is a local mirror. Rebuild with `node tools/uoou-index.mjs build`; the
...
`node tools/mzp-index.mjs build`; the ministry's robots.txt asks for ten seconds
...
1. **Query Czech sources in Czech.** `cz_guidance_search`, `cz_us_search`,
   ...
   subjects that list covers: extend it in `lib/czsearch.js` whenever a new area
...
The full procedure, with a worked example, is the `cz-legal-research` skill —
...
Google access is already authorised — `~/.claude_google_token.json`, Drive and
Docs scopes, shared with the scientific-writer project. No OAuth step is needed.

`tools/gdoc.py` is the working tool:

| Task | Command |
| --- | --- |
| Read a draft with comments anchored to their spans, plus tracked changes | `read <DOC_ID>` |
| Just the comment threads, with character offsets | `comments <DOC_ID>` |
| Surgical replacement, then reply and resolve | `edit <DOC_ID> --old '<json>' --new '<json>' --resolve-comment ID --reply TEXT` |
| Create a Doc from a markdown file | `create --title T --from-md FILE` |
| Turn existing text into a hyperlink | `link <DOC_ID> --text '<json>' --url URL` |

`edit` refuses when the target text is not unique, rather than silently changing
several places. Never rewrite a whole document to fix one comment: change the
commented span, leave everything else — including edits the owner made — alone.
...
in the source file becomes a real link in the Doc. This is what replaces the
Zotero-field-code approach used for academic work: for legislation and case law a
link to e-Sbírka, EUR-Lex or NALUS is better than a bibliography entry, because
the reader can check it.

**Always run `legal_cite` before writing a citation into a document.** It returns
the citation in both languages, the official URL, a ready-to-paste markdown link,
and the wording the provision actually has. A citation that has not been through
`legal_cite` does not go in a draft.

Zotero is for the other half: commentary, textbooks, articles. Cite those in the
ordinary author–year way and keep the item in the `law` collection.

## Czech legal workflows (`pravo-skills-cz`)

Nine Czech-language commands — contract review, NDA triage, compliance check,
vendor KYC, negotiation prep, risk assessment, template responses, briefings.
Adapted from DirectCase's MIT-licensed pack and rewired to `lex`; see
`plugins/pravo-skills-cz/NOTICE.md`.

Their substantive content is **not** verified law. It is a structure to fill.
Everything cited out of them goes through `legal_cite` first, and the
jurisdiction rule above still applies to anything they assert.

## Knowledge base (`vault/`)

`vault/` is an Obsidian vault. See `vault/README.md` for folder semantics and
`vault/90-Templates/` for note templates.

When asked to record something, write a note — do not dump it in chat only.

Note quality bar:
- English, professional register, direct. No filler, no hedging padding, no
  "it is important to note that".
- Structured so a future reader can re-orient in 30 seconds: what the rule is,
  what it means in practice, what to watch out for.
- Practical: written for someone who will come back to it while dealing with a
  similar problem, not for an exam.
- Sourced: every substantive claim has a citation with a link and the date read.
- Frontmatter filled in properly, `[[wikilinks]]` to related notes, tags from the
  controlled list in `vault/README.md`.

Do not create near-duplicate notes. Check `vault/` first; extend the existing note
if one covers the ground.
...
Full shape and examples: `vault/50-Handbook/Report format.md`.
...
`sources/` is for course materials, textbook extracts and case-law collections you
supply yourself. It is gitignored, because most of it will be third-party material
you have no right to redistribute. Treat anything in it as a **secondary source**:
useful for doctrine and for finding the right case, but verify every statutory
wording against `lex` before quoting it.

## Personal and shared configuration

| File | Tracked by git? | Holds |
| --- | --- | --- |
| `CLAUDE.md` (this file) | yes | the rules, identical for everyone |
| `PROFILE.md` | **no** | who you are, what you practise, your standing, your integrations |
| `vault/50-Handbook/Maintenance log.md` | **no** | what you last refreshed and when |
| `.claude/settings.local.json` | **no** | your local Claude Code settings |
| `.data/`, `.cache/` | **no** | built indexes and the HTTP cache, both rebuildable |

When you pull an update to this repository, `CLAUDE.md` may change and your
`PROFILE.md` will not be touched.
