...
1. **KYC protistrany** v obchodním rejstříku přes `lex` (existence, statutární orgán, oprávnění k podpisu, insolvence/likvidace).
...
3. **Ověření judikatury** ke sporným ustanovením v české soudní praxi (NS, ÚS, NSS) přes `lex`.
...
### Krok 2.5: KYC ověření protistrany přes `lex`

Pokud je v Kroku 0 potvrzeno použití `lex`, **před analýzou doložek** ověř protistranu v **českém obchodním rejstříku přes `lex` konektor**. Vyhledávání v OR nemusíš (a nemáš) provádět ručně mimo `lex` — konektor poskytuje přímý přístup k aktuálním datům z obchodního, živnostenského a insolvenčního rejstříku v rámci jednoho dotazu.
...
Pomocí **`lex`** vyhledej protistranu v obchodním rejstříku podle obchodního názvu nebo IČO uvedeného v hlavičce smlouvy a získej výpis. Z výpisu ověř a porovnej s hlavičkou smlouvy:
...
Z výpisu z OR (stejný dotaz na `lex` jako v 2.5.1) zjisti **statutární orgán** a **způsob jednání za společnost**. Typické varianty:
...
Ze stejného výpisu z `lex` (sekce z insolvenčního rejstříku a údaj o probíhající likvidaci) ověř:

- **Probíhající insolvenční řízení** — pokud `lex` uvádí insolvenci, smlouva typicky nesmí být uzavřena bez souhlasu insolvenčního správce; hrozí neúčinnost právních úkonů dle § 240–241 InsZ.
...
#### Využití `lex` pro autoritativní kontext

Pokud je v Kroku 0 potvrzeno použití `lex`, použij jej k doplnění analýzy o aktuální českou judikaturu, zákonná ustanovení a regulatorní stanoviska. Cílem je, aby každé ČERVENÉ a ŽLUTÉ označení bylo podloženo aktuální soudní praxí, nikoli pouze obecným tvrzením.

##### Povinná judikatorní rešerše

Ověř relevantní judikaturu v české soudní praxi (NS, ÚS, NSS) přes `lex`. **Jeden vyhledávací dotaz typicky pokryje všechny související doložky** — všechno z odpovědnosti, sankcí a lhůt sloučíš do jedné rešerše. Pokud má smlouva tematicky úplně různé okruhy (odpovědnost vs. arbitráž vs. licenční IP), můžeš rozdělit do více rešerší — **maximálně tři za celou revizi smlouvy**. Témata, která mají být pokryta (jsou-li ve smlouvě relevantní):
...
> **Pravidlo: maximálně tři rešerše na revizi smlouvy.** Pro každý druh zdroje (judikatura, zákony, regulatorika) můžeš v rámci jedné revize provést **nejvýše tři dotazy**. Dotazy formuluj komplexně a tematicky příbuzné okruhy spojuj do jednoho. Více rešerší použij jen tehdy, jsou-li ve smlouvě tematicky úplně různé okruhy, které do jednoho dotazu logicky nepatří.
>
> ⚙ **Pro AI agenta:** Konkrétní nástroje jsou `search_caselaw_parallel` (judikatura), `search_law_parallel` (zákony), `search_regulatory_parallel` (regulatorika). Limit max 3 volání platí pro každý nástroj samostatně.
...
→ proveď **doplňující rešerši judikatury** s úzce zaměřeným dotazem na konkrétní doložku (mimo povinnou rešerši v předchozí sekci). Pamatuj na limit maximálně tří rešerší judikatury na celou revizi — pokud jsi povinnou rešerši zvládl jedním dotazem, zbývají ti další dva pro tyto specifické případy. Citaci promítni do odůvodnění redline ve formě *„V případě sporu by soud podle linie NS sp. zn. … pravděpodobně …"* nebo *„NS opakovaně judikoval, že … (sp. zn. …)".*
...
> ⚙ **Pro AI agenta:** Konkrétní nástroje `lex`: `search_law_parallel` (zákony), `search_regulatory_parallel` (regulatorika), `get_law_detail` / `get_document` (plný text).

##### Citace ve výstupu

Každé tvrzení o judikatuře musí být ve výstupu doplněno identifikátorem rozhodnutí: **sp. zn. / ECLI / č. Sb. + datum**. Pokud se citace odvolává na konkrétní právní větu, uveď ji v uvozovkách. **Bez identifikátoru z `lex` citaci nepoužívej** — hrozí chybná nebo neexistující citace.
...
[Volitelně, je-li z `lex`] Judikatura: NS sp. zn. … — [stručný princip]
```
...
- volitelně judikaturu z `lex` (sp. zn. + krátký princip).
...
**Priorita**: Must-have / Should-have / Nice-to-have
[Volitelně z `lex`] **Judikatura**: NS sp. zn. … — [stručný princip]
