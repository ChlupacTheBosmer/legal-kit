...
if (!lowerBuilt) {
  console.log("  SKIP  lower-court index not built (node tools/justice-index.mjs build --from 2025-01-01)");
} else {
  {
...
console.log("\n== MŽP guidance (Ministry of the Environment) ==");
{
  ...
  if (r.isError && /has not been built yet/.test(r.body)) {
    console.log("  SKIP  MŽP index not built (node tools/mzp-index.mjs build)");
  } else {
